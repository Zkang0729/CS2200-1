...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
    ...
    pte_t *ptEntry = (pte_t *)(page_table + VPN); // &(page_table[VPN]);
    ...
    pfn_t newPFN = ptEntry -> pfn;
    fte_t *ftEntry = (fte_t *)(frame_table + newPFN);// &(frame_table[ptEntry -> pfn]);
    /*
    ...
    */
    ...
    paddr_t physicalAddr = ((paddr_t)newPFN << OFFSET_LEN) | offset;
...
void proc_cleanup(pcb_t *proc) {
    ...
    pte_t *curr_ptable = (pte_t *)(mem + (proc -> saved_ptbr) * PAGE_SIZE);

    /* Iterate the page table and clean up each valid page */
    for (size_t i = 0; i < NUM_PAGES; i++) {
        pte_t *ptEntry = &(curr_ptable[i]);
        if (ptEntry -> valid) {
